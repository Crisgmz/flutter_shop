-- ============================================================================
-- Pago mixto / dividido en el checkout
-- ============================================================================
-- checkout_sale_transactional ahora acepta p_payments (arreglo jsonb de
-- {method, amount}). Cuando viene, registra una fila de pago por método y
-- valida que la suma cuadre EXACTO con el total (todo pagado, dividido entre
-- métodos). Si p_payments es null, se comporta como antes (un solo pago por
-- el total). Las ventas a crédito siguen su flujo (no usan p_payments).
--
-- También agrega el valor 'other' (Otro) al enum de métodos de pago.
--
-- Ejecutar en el SQL Editor de Supabase. NO envolver en una transacción
-- explícita (el ALTER TYPE ... ADD VALUE corre mejor con autocommit).
-- ============================================================================

-- 1) Nuevo método de pago "Otro".
alter type public.payment_method add value if not exists 'other';

-- 2) Reemplazar la versión anterior (8 args) por la nueva (9 args).
...
begin
  ...
  if not p_as_credit then
    if p_payments is not null
      ...
      -- Pago mixto: validar que la suma cuadre con el total y registrar una
      -- fila de pago por método.
      ...
      if round(v_pay_sum, 2) <> round(v_total_amount, 2) then
        raise exception 'Los pagos (%) deben sumar el total (%).',
      ...
      -- Pago único (compatibilidad con el flujo anterior).
      insert into public.payments (
        branch_id, sale_id, client_id, cash_session_id, payment_method,
        amount, paid_at, reference, notes
      ) values (
