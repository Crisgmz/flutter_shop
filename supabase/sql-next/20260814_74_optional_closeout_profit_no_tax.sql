-- ============================================================================
-- Migración 74 — OPCIONAL — "Beneficios" del cierre del día sin ITBIS
-- ============================================================================
-- Dos arreglos en public.dashboard_v2_closeout, ambos visibles en la pantalla
-- "Cierre del día" (/panel/cierre):
--
-- 1) 'sales_total_no_tax' ("Ventas totales (sin impuestos)") salía en RD$ 0.00
--    porque se calculaba con `taxable_amount + exempt_amount`, dos columnas que
--    el checkout no llena (quedan en 0). Ahora usa esas columnas cuando traen
--    algo y, si no, cae a `subtotal`, que sí está siempre poblado.
--
-- 2) 'profit' ("Beneficios") se calculaba como total_amount − COGS, o sea
--    CON el ITBIS incluido. El ITBIS no es ganancia: es un impuesto que se le
--    cobra al cliente y se le paga a la DGII. Ahora es subtotal − COGS, que es
--    la ganancia real y además coincide con la columna "Ganancia" del Historial
--    de ventas y con el bloque nuevo "Venta por caja" de esta misma pantalla.
--
-- EFECTO VISIBLE: el número de "Beneficios" BAJA. La diferencia es exactamente
-- el ITBIS de las ventas del día. Ejemplo: venta de 23,600 (subtotal 20,000 +
-- ITBIS 3,600) con costo 10,000 → antes 13,600, ahora 10,000.
--
-- Si se prefiere seguir viendo el número viejo, NO correr esta migración: el
-- resto del sistema funciona igual. La única consecuencia de no correrla es que
-- "Beneficios" y el bloque "Venta por caja" muestran cifras distintas en la
-- misma pantalla.
--
-- El resto de la función queda IDÉNTICA a 20260509_12_closeout_returns_fix.sql
-- (mismo filtro status = 'completed', misma zona horaria America/Santo_Domingo,
-- mismos bloques de crédito, devoluciones, compras, gastos y caja).
--
...
begin
  ...
  select jsonb_build_object(
    'sales_total_no_tax', coalesce(sum(
      case when coalesce(s.taxable_amount, 0) + coalesce(s.exempt_amount, 0) > 0
           then s.taxable_amount + s.exempt_amount
           else s.subtotal
      end), 0)::numeric(14,2),
    'sales_total_with_tax', coalesce(sum(s.total_amount), 0)::numeric(14,2),
    'profit', coalesce(sum(s.subtotal), 0)::numeric(14,2)
