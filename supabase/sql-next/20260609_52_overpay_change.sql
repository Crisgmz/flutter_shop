-- ============================================================================
-- Sobrepago + cambio en el checkout
-- ============================================================================
-- Permite que el cliente pague de MÁS (los pagos pueden exceder el total). El
-- exceso es el "cambio" a devolver, que se guarda en sales.change_amount.
-- La caja descuenta ese cambio del efectivo esperado (lo calcula la app).
--
-- Reglas: los pagos no pueden ser MENORES que el total (eso sería crédito).
-- Si suman igual o más, la venta queda completada (paid_amount = total) y el
-- excedente se registra como change_amount.
--
-- Ejecutar en el SQL Editor de Supabase. NO envolver en transacción explícita.
-- ============================================================================

-- 1) Columna para el cambio devuelto.
alter table public.sales
  add column if not exists change_amount numeric(14,2) not null default 0;

-- 2) Reemplazar el checkout (misma firma de 9 args que la migración 50).
...
begin
  ...
  -- Pago mixto: validar que cubra el total y calcular el cambio (exceso).
  ...
  if not p_as_credit then
    if v_has_split then
      -- Registrar una fila de pago por método (tal como se cobró).
      for v_pay in
        select
